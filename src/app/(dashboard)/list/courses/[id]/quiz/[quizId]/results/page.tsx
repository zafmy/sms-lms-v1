...
const QuizResultsPage = async ({
    ...
    return (
      <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
          ...
          <span>/</span>
          <span className="text-gray-700 font-medium">{tq("results")}</span>
        </div>

        <h1 className="text-2xl font-semibold mb-6">
          {tq("resultsTitle", { title: quiz.title })}
        </h1>
        ...
        {latestAttempt ? (
          ...
          <p className="text-gray-500">{tq("noSubmittedAttempts")}</p>
        )}

        {/* Previous attempts */}
        {attempts.filter((a) => a.submittedAt).length > 1 && (
          <div className="mt-8">
            <h2 className="text-lg font-semibold mb-3">{tq("allAttempts")}</h2>
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      ...
      <h1 className="text-2xl font-semibold mb-6">
        {tq("resultsOverview", { title: quiz.title })}
      </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-blue-50 rounded-lg p-4 text-center">
          ...
          <div className="text-sm text-blue-600">{tq("averageScore")}</div>
        </div>
        <div className="bg-green-50 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-green-700">{passRate}%</div>
          <div className="text-sm text-green-600">{tq("passRate")}</div>
        </div>
        <div className="bg-purple-50 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-purple-700">
            {totalAttempts}
          </div>
          <div className="text-sm text-purple-600">{tq("totalAttempts")}</div>
        </div>
      </div>

      {/* PER-STUDENT BREAKDOWN */}
      <h2 className="text-lg font-semibold mb-3">{tq("studentResults")}</h2>
      {studentMap.size === 0 ? (
        <p className="text-gray-400 text-sm">{tq("noAttemptsYet")}</p>
      ) : (
        <div className="flex flex-col gap-2">
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    <span className="text-gray-500">
                      {tq("attemptCount", {
                        count: studentData.attempts.length,
                        suffix: studentData.attempts.length !== 1 ? "s" : "",
                      })}
                    </span>
                    <span>
                      {tq("bestScore", {
                        score: bestAttempt.score ?? 0,
                        max: bestAttempt.maxScore ?? 0,
                    ...
                    >
                      {bestAttempt.passed ? tq("passed") : tq("failed")}
                    </span>
                  </div>
                </div>
              );
